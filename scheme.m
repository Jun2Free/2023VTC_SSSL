...
figure
xlabel('X-axis (meters)', FontSize = 14);
ylabel('y-axis (meters)', FontSize = 14);
zlabel('Altitude (meters)', FontSize = 14);
p1 = plot3(l_DR(:,1), l_DR(:,2), l_DR(:,3), 'k-.o'); hold on; grid on;  % Drone Trajectory
p2 = plot3(est_T(2:size(est_T),1), est_T(2:size(est_T),2), est_T(2:size(est_T),3), 'b:x', 'MarkerSize',5); hold on;   % Estimated Location of the target
plot3(l_TG(1), l_TG(2), l_TG(3), 'pentagram', 'MarkerSize',10,'MarkerEdgeColor','r');   % Target Location
%p1.MarkerIndices = 1:2:length(l_DR);
p2.MarkerIndices = 1:5:length(est_T);
xlim([-10 350]);
ylim([0 350]);
zlim([0 4]);


legend('UAV Predefined Trajectory', 'Estimated Target Location over Time', 'True Target Location (Signal Source)', FontSize = 14);
