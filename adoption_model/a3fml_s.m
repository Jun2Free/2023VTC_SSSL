...
while tot_num_localization <= 1
   ...
   %% Distance Estimation
    ...
    while index_3 < matSize
        ...
        d(2) = dist_meas(round(index_3/2));

        A = [2*(l_DR(1,1) - l_DR(round(index_3/2),1)) 2*(l_DR(1,2) - l_DR(round(index_3/2),2)) 2*(l_DR(1,3) - l_DR(round(index_3/2),3));
            2*(l_DR(1,1) - l_DR(index_3,1)) 2*(l_DR(1,2) - l_DR(index_3,2)) 2*(l_DR(1,3) - l_DR(index_3,3))];
        B = [l_DR(1,1)^2 - l_DR(round(index_3/2),1)^2 + l_DR(1,2)^2 - l_DR(round(index_3/2),2)^2 + l_DR(1,3)^2 - l_DR(round(index_3/2),3)^2 - d(1)^2 + d(2)^2;
